...
<jsp:include page="includes/admin-header.jsp"></jsp:include>
    ...
    <style>
        ...
        body { background: linear-gradient(135deg, #f0fdf4 0%, #e2eeff 100%); color: #2d3436; font-family: 'Inter', system-ui, -apple-system, sans-serif; }
        .banner-container { height: 250px; border-radius: 24px; overflow: hidden; position: relative; box-shadow: 0 20px 40px rgba(17, 153, 142, 0.15); }
        .banner-img { width: 100%; height: 100%; object-fit: cover; background-image: url('IMG/porche.jpg'); background-size: cover; background-position: center; filter: brightness(0.5); }
        .banner-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(11, 44, 30, 0.7) 100%); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; }
        .money-title { background: linear-gradient(45deg, #11998e, #38ef7d); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; font-size: 3rem; margin-bottom: 5px; text-shadow: 0 5px 15px rgba(56, 239, 125, 0.2); }
        .aero-card { background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(20px); border: 1px solid rgba(17, 153, 142, 0.1); border-radius: 20px; box-shadow: 0 15px 35px rgba(0, 0, 0, 0.05); }
        .aero-input { background: #ffffff !important; border: 1px solid #c8e6c9 !important; border-radius: 14px !important; padding: 14px 18px; transition: all 0.3s ease; }
        .aero-input:focus { border-color: #11998e !important; box-shadow: 0 0 0 4px rgba(17, 153, 142, 0.1) !important; }
        .btn-money-gradient { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); border: none; color: #fff; font-weight: 700; border-radius: 14px; padding: 14px 30px; box-shadow: 0 8px 20px rgba(17, 153, 142, 0.2); transition: all 0.3s ease; text-transform: uppercase; letter-spacing: 1px; }
        .btn-money-gradient:hover { transform: translateY(-2px); box-shadow: 0 12px 25px rgba(17, 153, 142, 0.3); }
        .aero-table thead th { background: #f1fcf4; color: #11998e; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; padding: 20px; border: none; }
        .aero-table td { padding: 25px 20px; border-bottom: 1px solid #e8f5e9; }
        .aero-table tr:hover { background-color: rgba(56, 239, 125, 0.03); }
        .id-tag { background: #eafaf1; color: #11998e; padding: 6px 14px; border-radius: 10px; font-weight: 800; font-family: 'JetBrains Mono', monospace; font-size: 0.9rem; border: 1px solid rgba(17, 153, 142, 0.1); }
    </style>
    ...
    <div class="container-fluid py-5 px-5">
        ...
        <%-- FORM THÊM MỚI MỞ RỘNG (ĐÃ THÊM THUỘC TÍNH ENCTYPE ĐỂ UPLOAD FILE) --%>
        <div class="aero-card p-4 mb-5 border-start border-success border-5">
            ...
            <form action="PaymentMethodController" method="POST" enctype="multipart/form-data">
                ...
                <%-- KHI CHỌN QR THÌ HIỆN KHUNG NÀY ĐỂ NHẬP THÔNG TIN NGÂN HÀNG --%>
                <div id="addBankInfoBox" class="row g-3 p-3 bg-success bg-opacity-10 rounded-3 mb-4 border border-success border-opacity-25">
                    ...
                    <div class="col-md-3">
                        <%-- CHỖ NÀY ĐÃ SỬA THÀNH INPUT FILE --%>
...
<%-- MODAL CHỈNH SỬA (ĐÃ THÊM ENCTYPE VÀ NÚT CHỌN FILE) --%>
<c:forEach items="${listPaymentMethod}" var="pm">
                    ...
                    <div class="modal-body text-start">
                        ...
                        <div id="editBankInfoBox${pm.methodID}" class="row g-3 p-3 bg-success bg-opacity-10 rounded-3" style="display: ${pm.methodCode == 'QR' ? 'flex' : 'none'};">
                            ...
                            <div class="col-md-6">
                                ...
                                <%-- CHỖ NÀY ĐÃ SỬA THÀNH INPUT FILE VÀ THÊM THẺ HIDDEN --%>
...
<script>
    ...
    // SCRIPT ĐỂ TỰ ĐỘNG ẨN HIỆN FORM NGÂN HÀNG KHI CHỌN "QR"
