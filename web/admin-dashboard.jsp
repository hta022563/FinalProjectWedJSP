...
<jsp:include page="includes/admin-header.jsp"></jsp:include>
    ...
    <style>
        :root { --m-cyan: #06b6d4; --m-indigo: #6366f1; --dark-void: #020617; --m-glass: rgba(15, 23, 42, 0.8); }
        html, body, .grand-wrapper, .main-viewport, .container-fluid { background-color: var(--dark-void) !important; color: #f1f5f9 !important; margin: 0 !important; padding: 0 !important; }
        .grand-wrapper { display: flex; min-height: 100vh; width: 100vw; background: var(--dark-void) !important; }
        .main-viewport { flex: 1; min-width: 0; padding: 40px !important; background: radial-gradient(circle at top right, rgba(6, 182, 212, 0.05), transparent 40%) !important; }
        .terminal-hero { position: relative; border-radius: 35px; padding: 60px; margin-bottom: 40px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); box-shadow: 0 25px 50px rgba(0,0,0,0.8); }
        .hero-bg-car { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: url('IMG/Por.jpg'); background-size: cover; background-position: center; z-index: 0; filter: brightness(0.2) contrast(1.2); transform: scale(1.02); transition: 15s linear; }
        .terminal-hero:hover .hero-bg-car { transform: scale(1.1); }
        .hero-mask { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(90deg, #2e3241 30%, transparent); z-index: 1; }
        .hero-content { position: relative; z-index: 2; width: 100%; }
        .hud-clock { font-family: 'JetBrains Mono', monospace; font-size: 4rem; font-weight: 800; letter-spacing: -2px; background: linear-gradient(to bottom right, #fff, #94a3b8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .cyber-metric { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 25px; padding: 35px 30px; transition: 0.3s; }
        .cyber-metric:hover { border-color: var(--m-cyan); transform: translateY(-5px); }
        .module-panel { background: var(--m-glass); border-radius: 35px; border: 1px solid rgba(255,255,255,0.05); padding: 35px; }
            ...
            <div class="row g-4 mb-5 text-center">
                <div class="col-md-3">
                    <div class="cyber-metric">
                        ...
                        <c:choose>
                            <c:when test="${totalAccessories > 0}"><fmt:formatNumber value="${totalAccessories}" type="number" pattern="#,###"/></c:when>
                ...
                <div class="col-md-3">
                    <div class="cyber-metric">
                        ...
                        <h2 class="fw-bold m-0 text-white"><fmt:formatNumber value="${totalStock}" type="number" pattern="#,###"/></h2>
                ...
                <div class="col-md-3">
                    <div class="cyber-metric">
                        ...
                        <h2 class="fw-bold m-0 text-white"><fmt:formatNumber value="${totalCustomers}" type="number" pattern="#,###"/></h2>
                                        ...
                                        <div class="log-row d-flex align-items-center p-3 rounded-4 mb-3 position-relative">
                                            ...
                                            <div class="d-flex flex-column align-items-end justify-content-center ms-3">
                                                ...
                                                <div class="log-actions mt-2 d-flex gap-2">
                                                    <button class="btn btn-sm btn-outline-info rounded-circle d-flex align-items-center justify-content-center" style="width: 28px; height: 28px;" data-bs-toggle="modal" data-bs-target="#editLogModal" onclick="prepareEditModal('${act.logId}', '${act.title}', '${act.type}', '${act.referenceCode}', '${act.amount}')"><i class="fa-solid fa-pen" style="font-size: 0.7rem;"></i></button>
                                                    <a href="MainController?target=Dashboard&action=deleteLog&id=${act.logId}" class="btn btn-sm btn-outline-danger rounded-circle d-flex align-items-center justify-content-center" style="width: 28px; height: 28px;" onclick="return confirm('Cảnh báo: Xác nhận tiêu hủy dòng nhật ký hệ thống này?');"><i class="fa-solid fa-trash" style="font-size: 0.7rem;"></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </c:forEach>
                ...
                <div class="table-responsive cyber-feed pe-2" style="max-height: 450px; overflow-y: auto;">
                    <table class="table matrix-table mb-0">
                        <thead style="position: sticky; top: 0; z-index: 10; background: var(--dark-void); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.5);">
                            <tr>
                                <th class="text-start">MÃ HỢP ĐỒNG</th> <th class="text-center">NGÀY THIẾT LẬP</th> <th class="text-end">GIÁ TRỊ (VND)</th> <th class="text-center">TÌNH TRẠNG</th> <th class="text-center">TÁC VỤ ĐIỀU PHỐI</th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:choose>
        ...
        function refreshLogs() {
            fetch('MainController?target=Dashboard&type=ajax')
                    ...
                    .then(html => {
                        ...
                        if (html.trim() !== "" && feedList) { feedList.innerHTML = html; }
                    })
