...
<%
    ...
    try {
        ProductDAO daoHome = new ProductDAO();
        request.setAttribute("carG63", daoHome.getProductById(15));
        request.setAttribute("carAudi", daoHome.getProductById(8));
        request.setAttribute("carC300", daoHome.getProductById(5));
    } catch (Exception e) {
        System.out.println("Lỗi load xe trang chủ: " + e.getMessage());
    }
%>
...
<style>
    ...
    .btn-luxury { background: #FFD700; color: #000 !important; font-weight: 700; text-transform: uppercase; border: none; padding: 12px 35px; transition: 0.3s; font-size: 0.9rem; letter-spacing: 1px; }
    .btn-luxury:hover { background: #D4AF37; transform: translateY(-2px); }
    .btn-outline-luxury { color: #D4AF37; border: 1px solid #D4AF37; text-transform: uppercase; font-weight: 600; transition: 0.3s; background: transparent; padding: 10px 30px; font-size: 0.85rem; letter-spacing: 1px; }
    .btn-outline-luxury:hover { background: rgba(212, 175, 55, 0.1); color: #FFD700; border-color: #FFD700; }
    .hero-banner { position: relative; height: 85vh; min-height: 500px; background-image: url('IMG/505329.jpg'); background-size: cover; background-position: center; background-attachment: fixed; }
    .hero-overlay { position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(to right, rgba(0,0,0,0.95) 10%, rgba(0,0,0,0.4) 100%); }
    .luxury-card { background: #111; border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 10px; overflow: hidden; transition: 0.4s; }
    .luxury-card:hover { transform: translateY(-5px); border-color: rgba(212, 175, 55, 0.3); }
    .car-img-wrapper { overflow: hidden; }
    .car-img-wrapper img { transition: transform 0.6s ease; }
    .luxury-card:hover .car-img-wrapper img { transform: scale(1.05); }
    .test-drive-banner { background: linear-gradient(rgba(0,0,0,0.8), rgba(0,0,0,0.9)), url('IMG/car_s450.jpg') center/cover fixed; padding: 100px 0; border-top: 1px solid #333; }
    .process-box { text-align: center; padding: 30px 20px; position: relative; }
    .step-number { font-family: 'Playfair Display', serif; font-size: 5rem; font-weight: 700; color: rgba(212, 175, 55, 0.1); position: absolute; top: -10px; left: 50%; transform: translateX(-50%); z-index: 0; transition: 0.4s; }
    .process-content { position: relative; z-index: 1; margin-top: 40px; }
    .process-box:hover .step-number { color: rgba(212, 175, 55, 0.3); transform: translate(-50%, -10px); }
    .news-card { background: #111; border: 1px solid #222; border-radius: 8px; overflow: hidden; transition: 0.3s; }
...
<div class="container mt-5 pt-5 mb-5" id="showroom">
    ...
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card luxury-card h-100 pb-3">
                <div class="car-img-wrapper">
                    <img src="${carG63 != null ? carG63.imageURL : 'IMG/OIP (2).webp'}" class="card-img-top" alt="Mercedes-Benz G63" style="height:250px; object-fit:cover" onerror="this.src='IMG/logo.jpg'">
                </div>
                <div class="card-body text-center p-4">
                    <p class="text-secondary small mb-1" style="font-size: 0.8rem;">SUV | 2024</p>
                    <h5 class="card-title text-white fw-bold mb-3">${carG63 != null ? carG63.productName : 'Mercedes-Benz G63'}</h5>
                    <h5 class="text-gold fw-bold mb-4">
                        <c:choose>
                            <c:when test="${carG63 != null}"><fmt:formatNumber value="${carG63.price}" type="number" pattern="#,###"/> VNĐ</c:when>
                            <c:otherwise>11,000,000,000 VNĐ</c:otherwise>
                        </c:choose>
                    </h5>
        ...
        <div class="col-md-4">
            <div class="card luxury-card h-100 pb-3">
                <div class="car-img-wrapper">
                    <img src="${carAudi != null ? carAudi.imageURL : 'IMG/OIP.webp'}" class="card-img-top" alt="Audi R8 V10" style="height:250px; object-fit:cover" onerror="this.src='IMG/logo.jpg'">
                </div>
                <div class="card-body text-center p-4">
                    <p class="text-secondary small mb-1" style="font-size: 0.8rem;">Sport | 2023</p>
                    <h5 class="card-title text-white fw-bold mb-3">${carAudi != null ? carAudi.productName : 'Audi R8 V10'}</h5>
                    <h5 class="text-gold fw-bold mb-4">
                        <c:choose>
                            <c:when test="${carAudi != null}"><fmt:formatNumber value="${carAudi.price}" type="number" pattern="#,###"/> VNĐ</c:when>
                            <c:otherwise>4,500,000,000 VNĐ</c:otherwise>
                        </c:choose>
                    </h5>
        ...
        <div class="col-md-4">
            <div class="card luxury-card h-100 pb-3">
                <div class="car-img-wrapper">
                    <img src="${carC300 != null ? carC300.imageURL : 'IMG/Mec300.webp'}" class="card-img-top" alt="Mercedes C300" style="height:250px; object-fit:cover" onerror="this.src='IMG/logo.jpg'">
                </div>
                <div class="card-body text-center p-4">
                    <p class="text-secondary small mb-1" style="font-size: 0.8rem;">Sedan | 2025</p>
                    <h5 class="card-title text-white fw-bold mb-3">${carC300 != null ? carC300.productName : 'Mercedes C300'}</h5>
                    <h5 class="text-gold fw-bold mb-4">
                        <c:choose>
                            <c:when test="${carC300 != null}"><fmt:formatNumber value="${carC300.price}" type="number" pattern="#,###"/> VNĐ</c:when>
                            <c:otherwise>1,500,000,000 VNĐ</c:otherwise>
                        </c:choose>
                    </h5>
...
<c:if test="${not empty sessionScope.msg}">
    <script>
        Swal.fire({ 
            ...
            html: '<span style="color: #e0e0e0; font-family: \'Montserrat\', sans-serif;">${sessionScope.msg}</span>', 
            icon: 'success', iconColor: '#D4AF37', background: '#121212', 
            customClass: { confirmButton: 'btn btn-warning px-4 py-2 rounded-pill fw-bold text-dark' }, 
            buttonsStyling: false, confirmButtonText: 'Tuyệt vời' 
        });
    </script>
...
<c:if test="${not empty sessionScope.msgError}">
    <script>
        Swal.fire({ 
            title: '<span style="font-family: \'Playfair Display\', serif; color: #ff4d4d;">CẢNH BÁO!</span>', 
            html: '<span style="color: #e0e0e0; font-family: \'Montserrat\', sans-serif;">${sessionScope.msgError}</span>', 
            icon: 'error', background: '#121212', 
            customClass: { confirmButton: 'btn btn-outline-danger px-4 py-2 rounded-pill fw-bold' }, 
